...
import top.ilianpay.util.StreamUtils;
...
public class HttpURLConnectionClient extends HttpClient {
    ...
    @Override
    public APIIlianPayResponse request(APIIlianPayRequest request) throws APIConnectionException {
        HttpURLConnection conn = null;

        try {
            conn = createIlianPayConnection(request);
            ...
            return new APIIlianPayResponse(responseCode, responseBody, headers);
        } catch (IOException e) {
    ...
    static HttpHeaders getHeaders(APIIlianPayRequest request) {
        ...
        userAgentHeadersMap.put(
                "X-IlianPay-Client-User-Agent", Collections.singletonList(buildXIlianPayClientUserAgentString(request.getOptions().getVersion())));

        return request.getHeaders().withAdditionalHeaders(userAgentHeadersMap);
    }

    private static HttpURLConnection createIlianPayConnection(APIIlianPayRequest request)
            throws IOException {
