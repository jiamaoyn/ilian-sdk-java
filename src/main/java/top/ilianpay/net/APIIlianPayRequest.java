...
import top.ilianpay.util.IlianPayKit;
import top.ilianpay.util.StringUtils;
 ...
 * API请求
 */
public class APIIlianPayRequest {
    /**
    ...
    public APIIlianPayRequest(
        ...
        try {
            ...
            throw new APIConnectionException(
                    String.format(
                            "请求IlianPay(%s)异常,请检查网络或重试.异常信息:%s",
    ...
    public static URL buildURLWithSign(String url, Map<String, Object> params, RequestOptions options) throws APIConnectionException {
        ...
        params.put(IlianPay.API_VERSION_NAME, options.getVersion());
        params.put(IlianPay.API_SIGN_TYPE_NAME, options.getSignType());
        String requestTime = currentTimeString();
        params.put(IlianPay.API_REQ_TIME_NAME, requestTime);
        String signature;
        try {
            signature = buildIlianPaySignature(params, options);
        } catch (IOException e) {
            throw new APIConnectionException("生成IlianPay请求签名异常", e);
        }

        if (signature != null) {
            params.put(IlianPay.API_SIGN_NAME, signature);
        }
        ...
        try {
            ...
            throw new APIConnectionException("生成 IlianPay 请求URL异常", e);
        }
    }

    private static HttpContent buildContent (
            APIResource.RequestMethod method, Map<String, Object> params, RequestOptions options) throws IlianPayException {
    ...
    protected static String buildIlianPaySignature(Map<String, Object> params, RequestOptions options)
            throws IOException {

        String signType = options.getSignType();
        if("MD5".equalsIgnoreCase(signType)) {
            return IlianPayKit.getSign(params, options.getApiKey());
